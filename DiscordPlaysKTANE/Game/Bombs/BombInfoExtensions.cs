...
namespace DiscordPlaysKTANE.Game.Bombs {
    ...
    /// <summary>
    /// Some helper extensions methods for the BombInfo class.
    /// </summary>
    public static class BombInfoExtensions {
        #region JSON Types
        ...
        private static IEnumerable<T> GetJSONEntries<T>(BombInfo bombInfo, string queryKey, string queryInfo)
            where T : new() {
            return bombInfo.GetWidgetQueryResponses(queryKey, queryInfo).Select(delegate (string queryEntry) {
                return JsonConvert.DeserializeObject<T>(queryEntry);
            });
        }

        private static IEnumerable<IndicatorJSON> GetIndicatorEntries(BombInfo bombInfo) {
            return GetJSONEntries<IndicatorJSON>(bombInfo, BombInfo.QUERYKEY_GET_INDICATOR, null);
        }

        private static IEnumerable<ColorIndicatorJSON> GetColorIndicatorEntries(BombInfo bombInfo) {
            return GetJSONEntries<ColorIndicatorJSON>(bombInfo, BombInfo.QUERYKEY_GET_INDICATOR + "Color", null);
        }

        private static IEnumerable<BatteryJSON> GetBatteryEntries(BombInfo bombInfo) {
            return GetJSONEntries<BatteryJSON>(bombInfo, BombInfo.QUERYKEY_GET_BATTERIES, null);
        }

        private static IEnumerable<PortsJSON> GetPortEntries(BombInfo bombInfo) {
            return GetJSONEntries<PortsJSON>(bombInfo, BombInfo.QUERYKEY_GET_PORTS, null);
        }

        private static IEnumerable<SerialNumberJSON> GetSerialNumberEntries(BombInfo bombInfo) {
            return GetJSONEntries<SerialNumberJSON>(bombInfo, BombInfo.QUERYKEY_GET_SERIAL_NUMBER, null);
        }

        private static IEnumerable<TwoFactorJSON> GetTwoFactorEntries(BombInfo bombInfo) {
        ...
        public static bool IsIndicatorPresent(this BombInfo bombInfo, Indicator indicatorLabel) {
            return bombInfo.IsIndicatorPresent(indicatorLabel.ToString());
        }

        public static bool IsIndicatorPresent(this BombInfo bombInfo, string indicatorLabel) {
            return GetIndicatorEntries(bombInfo).Any((x) => indicatorLabel.Equals(x.label));
        }

        public static bool IsIndicatorColored(this BombInfo bombInfo, Indicator indicatorLabel, string indicatorColor) {
            return IsIndicatorColored(bombInfo, indicatorLabel.ToString(), indicatorColor);
        }

        public static bool IsIndicatorColored(this BombInfo bombInfo, string indicatorLabel, string indicatorColor) {
        ...
        public static bool IsIndicatorOn(this BombInfo bombInfo, Indicator indicatorLabel) {
            return bombInfo.IsIndicatorOn(indicatorLabel.ToString());
        }

        public static bool IsIndicatorOn(this BombInfo bombInfo, string indicatorLabel) {
        ...
        public static IEnumerable<string> GetIndicators(this BombInfo bombInfo) {
            return GetIndicatorEntries(bombInfo).Select((x) => x.label);
        }

        public static IEnumerable<string> GetOnIndicators(this BombInfo bombInfo) {
        ...
        public static int GetBatteryCount(this BombInfo bombInfo, Battery batteryType) {
        ...
        public static IEnumerable<string> GetPorts(this BombInfo bombInfo) {
